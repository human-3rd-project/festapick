...
const SignupPage = () => {
  ...
  useEffect(() => {
    if (!verificationExpiresAt || verifiedEmail) {
      return;
    }
    ...
    setMessages((prevMessages) => ({
      ...
      verificationCode: {
        type: "error",
        text: "인증 시간이 만료되었습니다. 인증을 다시 요청해 주세요.",
      },
    }));
  ...
  const getFieldMessage = (fieldName, value, form) => {
    if (fieldName === "userId") {
      ...
      if (!isValidUserId(value))
        return {
          type: "error",
          text: "아이디는 영문, 숫자, 밑줄(_) 4~20자로 입력해 주세요.",
        };
    }
    ...
    if (fieldName === "nickname") {
      ...
      if (!isValidNickname(value))
        return {
          type: "error",
          text: "닉네임은 한글, 영문, 숫자 2~12자로 입력해 주세요.",
        };
    }
    ...
    if (fieldName === "email") {
      ...
      if (!isValidEmail(value))
        return { type: "error", text: "이메일 형식에 맞게 입력해 주세요." };
    }
    ...
    if (fieldName === "password") {
      ...
      if (!isValidPassword(value))
        return {
          type: "error",
          text: "비밀번호는 영문, 숫자, 특수문자를 포함해 8~20자로 입력해 주세요.",
        };
    }
    ...
    if (fieldName === "passwordConfirm") {
      ...
      if (!value)
        return { type: "error", text: "비밀번호를 다시 입력해 주세요." };
  ...
  const handleInputChange = (fieldName, e) => {
    ...
    const passwordConfirm =
      fieldName === "password" ? form.passwordConfirm?.value || "" : "";
    ...
    setMessages((prevMessages) => {
      ...
      if (fieldName === "password") {
        if (passwordConfirm) {
          nextMessages.passwordConfirm = getFieldMessage(
            "passwordConfirm",
            passwordConfirm,
            form,
          );
    ...
    if (fieldName === "userId" || fieldName === "nickname") {
      setCheckedFields((prevCheckedFields) => ({
        ...prevCheckedFields,
        [fieldName]:
          prevCheckedFields[fieldName] === value
            ? prevCheckedFields[fieldName]
            : "",
      }));
    }
  ...
  const handleFieldBlur = (fieldName, e) => {
    ...
    setMessages((prevMessages) => {
      ...
      if (fieldName === "password") {
        ...
        const passwordConfirmMessage = getFieldMessage(
          "passwordConfirm",
          passwordConfirm,
          form,
        );
  ...
  const handleCheck = async (fieldName, label, e) => {
    ...
    const checkApi =
      fieldName === "userId" ? AxiosApi.checkLoginId : AxiosApi.checkNickname;
    ...
    try {
      ...
      setFieldMessage(fieldName, {
        type: "error",
        text: getApiMessage(error, `${label} 중복 확인에 실패했습니다.`),
      });
    } finally {
  ...
  const handleEmailVerification = async (e) => {
    ...
    try {
      ...
      setFieldMessage("email", {
        type: "success",
        text: "인증코드를 발송했습니다. 메일함을 확인해 주세요.",
      });
      setFieldMessage("verificationCode", {
        type: "success",
        text: "메일로 받은 6자리 인증코드를 입력해 주세요.",
      });
    } catch (error) {
      ...
      setFieldMessage("email", {
        type: "error",
        text: getApiMessage(error, "인증코드 발송에 실패했습니다."),
      });
    } finally {
  ...
  const handleVerificationCodeCheck = async (e) => {
    ...
    const emailMessage = getFieldMessage("email", email, form);
    const codeMessage = getFieldMessage(
      "verificationCode",
      verificationCode,
      form,
    );
    ...
    if (checkedFields.email !== email || !verificationExpiresAt) {
      // 인증 요청을 보낸 이메일과 현재 입력 이메일이 다르면 같은 코드로 검증할 수 없습니다.
      setFieldMessage("verificationCode", {
        type: "error",
        text: "현재 이메일로 인증을 먼저 요청해 주세요.",
      });
      return;
    }
    ...
    try {
      ...
      if (
        form.email?.value.trim() !== email ||
        form.verificationCode?.value.trim() !== verificationCode
      ) {
        return;
      }
      ...
      setFieldMessage("verificationCode", {
        type: "error",
        text: getApiMessage(error, "인증코드가 일치하지 않습니다."),
      });
    } finally {
  ...
  const validateSubmit = (form) => {
    ...
    const fieldMessages = {
      ...
      verificationCode: getFieldMessage(
        "verificationCode",
        verificationCode,
        form,
      ),
      password: getFieldMessage("password", password, form),
      passwordConfirm: getFieldMessage(
        "passwordConfirm",
        passwordConfirm,
        form,
      ),
    ...
    if (!validationMessages.verificationCode && verifiedEmail !== email) {
      ...
      validationMessages.verificationCode = {
        type: "error",
  ...
  const handleSubmit = async (e) => {
    ...
    const hasError = Object.values(validationMessages).some(
    ...
    try {
      ...
      const fallbackMessage =
        "회원가입에 실패했습니다. 입력값과 이메일 인증 상태를 확인해 주세요.";
  ...
  const renderFieldMessage = (fieldName) => {
    ...
    return (
      <Styles.FieldMessage
        className={`${message?.type || ""} ${message ? "" : "empty"}`}
      >
        {message?.text || "메시지 영역"}
      </Styles.FieldMessage>
    );
  ...
  const formatRemainingTime = () => {
    const seconds = verificationExpiresAt
      ? remainingSeconds
        ...
        <Styles.GlassContainer>
          ...
          <Styles.Form onSubmit={handleSubmit} noValidate>
            <Styles.Field>
              ...
              <Styles.FieldWithButton>
                ...
                <Styles.GlassButton
                  ...
                  onClick={(e) => handleCheck("userId", "아이디", e)}
                >
            ...
            <Styles.Field>
              ...
              <Styles.FieldWithButton>
                ...
                <Styles.GlassButton
                  ...
                  onClick={(e) => handleCheck("nickname", "닉네임", e)}
                >
            ...
            <Styles.FieldGroup>
              ...
              {(verificationExpiresAt || verifiedEmail) && (
                <Styles.Field>
                  ...
                  <Styles.FieldWithButton>
                    <Styles.InputGroup>
                      <Styles.Input
                        ...
                        data-has-timer="true"
                        aria-invalid={
                          messages.verificationCode?.type === "error"
                        }
                        onChange={(e) =>
                          handleInputChange("verificationCode", e)
                        }
            ...
            {messages.terms && (
              <Styles.FieldMessage className={messages.terms.type}>
                {messages.terms.text}
              </Styles.FieldMessage>
            )}
